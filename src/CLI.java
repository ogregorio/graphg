...
public class CLI {
	...
	public void run(String command) {
		if (interpreter(command)) {
			if(DEBUG) System.out.println("Command recognized!");
		} else {
			if(DEBUG) System.out.println("Command not recognized!");
		}
	}
	...
	private boolean interpreter(String command) {
		...
		case "DEBUG_ON": return debugSet(code[0]);
		case "DEBUG_OFF": return debugSet(code[0]);
		case "CREATE": return matrixCreate(code[1]);
		case "ADD": return commandsDecode(code);
		case "PRINT": return matrix.print();
	...
	private boolean commandsDecode(String[] code) {
		...
		if(code[0].equalsIgnoreCase("ADD")) {
			...
			switch (code[1]) {
				case "VERTEX": return createVertex(code);
				case "V": return createVertex(code);
				case "EDGE": return createEdge(code);
				case "E": return createVertex(code);
				default: return false;
			}
		}
	...
	private boolean createEdge(String[] code) {
		...
		if(matrix.getClass() == MatrixLabeled.class)
			...
			if(DEBUG) System.out.println("Argument missing, try to explicitly place the label!");
		}
		return false;
	}
